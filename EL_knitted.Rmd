...
On the third day of the workshop, they presented an idea known as the “expected loss” curve (ELC) as an addition to the Cost Effectiveness Acceptability Curve (CEAC). After reading their paper on the topic (see [here](https://www.valueinhealthjournal.com/article/S1098-3015(19)30133-0/abstract)), I was really struggling to understand why this wasn’t recommended by all the major CEA guidelines? In fact, almost all of them recommend a CEAC but that only tells some of the story. So why not both?  
<br>
...
Let us suppose that you want to compare two strategies (Strategy A and Strategy B) to see which will be optimal for your company. Your head statistician informs you that Strategy A will be cost-effective 70% of the time and in the 70 times out of 100 that it is cost-effective, you stand to gain \$5 dollars each time or "you lose \$0". She then proceeds to tell you that for every time you are wrong (30% of the time, here) you stand to lose \$100. Your expected loss here would be \$30 (30% x \$100 = \$30). With that in mind, you also calculate the expected loss for Strategy B. Turns out it is only \$7! (\$7 here is arbitrary for the sake of example).
<br>

So here we have a strategy where _only_ knowing what is most likely to be cost-effective would mislead a decision maker to the incorrect decision (the one with a greater expected loss) and subject their company to unnecessary risks.
<br>

### Expected Loss vs. Cost-Effectiveness Acceptability
...
<span style="color:indianred">**Expected Loss Curves**</span> represent the average financial loss when a sample from the same PSA does *not* have the highest net monetary benefit.
<br>
...
<br> 
  ...
  + and what the expected financial loss (or risk) is if the chosen strategy turns out *not* to be cost-effective (Expected Loss)
<br>
...
<br>
<br>
Aand then our expected loss for Drug A or Drug B is just the average of all of those losses...  
<br>
<br>
...
Consider the following potential distributions of your incremental net benefit (the difference between Drug B and Drug A at each iteration), or INB.  
<br>
<br>
```{r, fig.align= "center"}
...
ggplot(s) +
  ...
  labs(title = "Distribution of Incremental Net Monetary Benefit",
       subtitle = "for three hypothetical INB distributions") +
...
All three hypothetical distributions represent scenarios where the given strategy is not 100% likely to be cost effective (we know this because we can see some of each distribution being < $0, which means that at least one sample of the PSA favors the other strategy). Of course, these distributions could be located anywhere on the x-axis but most often you’ll have something that looks like those pictured above (each strategy has some probability of being cost-effective).  
<br>
<br>
